 ...
 * @returns {*}
 */
export function getSnapshot(target, applyPostProcess) {
...
export function getMembers(target) {
    // check all arguments
    if (process.env.NODE_ENV !== "production") {
        var node_1 = getStateTreeNode(target);
        if (!(node_1.type instanceof ModelType))
    ...
    var reflected = {
        name: type.name,
        properties: tslib_1.__assign({}, type.properties),
        actions: [],
        volatile: [],
        views: []
    };
...
import { getStateTreeNode, isStateTreeNode, splitJsonPath, asArray, EMPTY_OBJECT, fail, isType, resolveNodeByPath, getRelativePathBetweenNodes, ModelType, freeze } from "../internal";
